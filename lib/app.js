...
app.get('/characters/:name', async (req, res) => {
  try {
    const chName = req.params.name;
    const data = await client.query('SELECT * from characters WHERE name=$1', [chName]);
...
app.put('/characters/:id', async (req, res) => {
  ...
  try {
    const data = await client.query(`
    UPDATE characters 
    SET name = $1, species = $2, role = $3, unique_power = $4, movie = $5, movie_year = $6, hand_drawn = $7, image = $8, gif = $9, owner_id = $10
    WHERE id = $11
    RETURNING *
    `,
      [
